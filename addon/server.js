...
function getPtCatalogDisplayName(group, sourceName, catalogName, catalogId, type) {
  ...
  if (group === "ptpt") {
    const sourceText = normalizeSearchText(
      `${sourceName || ""} ${catalogId || ""} ${catalogName || ""}`
    );

    if (/animacao|kidspt|kids pt/.test(sourceText)) {
      if (type === "series") return "🇵🇹 Animação PT-PT • Séries";
      if (type === "movie") return "🇵🇹 Animação PT-PT • Filmes";
      return "🇵🇹 Animação PT-PT";
    }
...
async function getPtExternalStreams(config, type, wrappedId) {
  ...
  try {
    ...
    const groupLabel =
      parsed.group === "ptpt"
        ? (/animacao|kidspt|kids pt/.test(normalizedSourceName)
            ? "Animação PT-PT"
            : "PT-PT")
        : parsed.group === "adult"
          ? "Adultos"
