...
app.get("/version", (req, res) => {
  ...
  return res.json({
    name: "PT•HUB",
    version: VERSION,
    build: "3.0.0-RC-PTLANG-STREAM-PROVIDERS-20260831"
  });
});

/* =========================================================
   PT•HUB CONFIG STORE — PERSISTENTE / SEM ESTADO
   =========================================================
   A configuração fica comprimida dentro do próprio token.
   Não depende da RAM ou do disco temporário do Render.
   ========================================================= */
...
function createPersistentConfigToken(config) {
  ...
  return CONFIG_TOKEN_PREFIX +
    zlib.deflateRawSync(Buffer.from(serialized, "utf8"), { level: 9 }).toString("base64url");
}

function getPersistentConfig(token) {
  const value = String(token || "");
  if (!value.startsWith(CONFIG_TOKEN_PREFIX)) return null;
  const payload = value.slice(CONFIG_TOKEN_PREFIX.length);
  return JSON.parse(
...
app.post("/config-store", (req, res) => {
  try {
    ...
    const token = createPersistentConfigToken(config);
    return res.json({ success: true, token, persistent: true });
  } catch (error) {
    console.error("Erro ao criar configuração persistente:", error.message);
    return res.status(error.message === "Configuração demasiado grande." ? 413 : 500).json({
      success: false,
      error: error.message || "Não foi possível criar a configuração."
    });
  }
});
...
function encodeConfig(config) {
 ...
 function decodeConfig(value) {
  try {
    ...
    if (String(value).startsWith("c_")) return null;
...
function validateConfig(config) {
 ...
 if (config.mode === "m3u") {
 if (!isValidHttpUrl(config.m3uUrl)) return "Indica um URL M3U válido.";
 }
...
/* =========================================================
   ...
   o utilizador indique na configuração e devolve todos os
   torrents/streams encontrados, sem limite por qualidade.
   ========================================================= */
...
function getExternalStreamProviderLabel(baseUrl) {
  const normalized = normalizeUrl(baseUrl).replace(/\/manifest\.json$/i, "");

  const known = PT_BUILTIN_ADDONS.find((addon) =>
    normalizeUrl(addon.manifestUrl).replace(/\/manifest\.json$/i, "") === normalized
  );

  return known?.name || normalized;
}

async function fetchExternalStreamSource(baseUrl, type, id, diagnostics = null) {
  ...
  const providerName = getExternalStreamProviderLabel(baseUrl);
  const url =
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    try {
      const response = await fetch(url, {
        ...
        headers: {
          "User-Agent":
            ...
            "(KHTML, like Gecko) Chrome/151.0 Safari/537.36 PT-HUB/" + VERSION,
      ...
      if (!response.ok) {
        ...
        console.log(
          `Streams PT•HUB: ${providerName} -> HTTP ${response.status}`
        );
      ...
      console.log(
        `Streams PT•HUB: ${providerName} -> HTTP ${response.status}, ${streams.length} streams`
      );
      ...
      console.log(
        `Streams PT•HUB: ${providerName} -> ${lastError}`
      );
...
async function getExternalStreams(config, type, id) {
  ...
  // Sem limite por qualidade: todos os resultados únicos de todos os addons são devolvidos.
  return dedupedStreams;

}


app.get("/debug/streams", async (req, res) => {
  try {
    ...
    if (!id) {
      return res.status(400).json({
        success: false,
        build: "3.0.0-RC-PTLANG-STREAM-PROVIDERS-20260831",
        error: "Falta id (ex.: tt0111161)"
      });
    }

    const diagnostics = [];
    const sources = getBuiltinStreamSources();

    await Promise.all(
      sources.map((source) =>
        fetchExternalStreamSource(source, type, id, diagnostics)
      )
    );
...
function normalizeSubtitleLanguage(value) {
  ...
  if (ptPt.has(normalized)) return "pt";
  if (ptBr.has(normalized)) return "pt-br";

  const map = {
    eng: "en", en: "en", english: "en",
    spa: "es", es: "es", spanish: "es", "español": "es", espanol: "es",
    fre: "fr", fra: "fr", fr: "fr", french: "fr", "français": "fr", francais: "fr",
    ...
    ron: "ro", rum: "ro", ro: "ro", romanian: "ro",
    ell: "el", gre: "el", el: "el", greek: "el",
  ...
  if (/^[a-z]{2}-[a-z]{2,3}$/.test(normalized)) return normalized;
