...
/* =========================================================
   IPTV — POSTER VERTICAL COM LOGO CONTIDO
   =========================================================
   Stremio/Nuvio fazem crop dos posters. Em vez de lhes entregar o logo
   diretamente, o PT•HUB cria um poster 2:3 e coloca o logo ao centro
   com margem interna. O logo original continua disponível no campo logo.
   ========================================================= */

const CHANNEL_POSTER_CACHE_TTL_MS = 24 * 60 * 60 * 1000;
const CHANNEL_POSTER_FETCH_TIMEOUT_MS = 12000;
const channelPosterCache = new Map();

function encodeChannelPosterTarget(url) {
  return Buffer.from(String(url || ""), "utf8").toString("base64url");
}

function decodeChannelPosterTarget(value) {
  try {
...
function buildChannelPosterUrl(req, logoUrl) {
  const source = String(logoUrl || PT_HUB_LOGO).trim() || PT_HUB_LOGO;
  const protocol = String(req.headers["x-forwarded-proto"] || req.protocol || "https")
    .split(",")[0].trim();
  const host = req.get("host");
  return `${protocol}://${host}/channel-poster/${encodeChannelPosterTarget(source)}.svg`;
}

function isIgnorableIptvChannelName(name) {
  const value = String(name || "").trim();
  return (
    /^(?:data|date)\s*[-:]\s*\d{4}-\d{2}-\d{2}$/i.test(value) ||
    /^(?:expira|expires?|validade)\s*[-:]\s*\d{4}-\d{2}-\d{2}$/i.test(value)
  );
}

function finalizeIPTVChannels(channels) {
  return (Array.isArray(channels) ? channels : [])
    .filter((channel) => !isIgnorableIptvChannelName(channel?.name));
}
...
function getChannelPosterLogoBox(dimensions) {
  const width = Number(dimensions?.width || 0);
  const height = Number(dimensions?.height || 0);
  const ratio = width > 0 && height > 0 ? width / height : 1.6;

  // Logos muito horizontais precisam de mais largura para não parecerem minúsculos.
  if (ratio >= 3.2) return { width: 430, height: 150 };
  if (ratio >= 2.2) return { width: 410, height: 180 };
  if (ratio >= 1.5) return { width: 370, height: 220 };
  if (ratio >= 1.0) return { width: 290, height: 260 };
  return { width: 235, height: 285 };
}

app.get("/channel-poster/:target.svg", async (req, res) => {
  const targetUrl = decodeChannelPosterTarget(req.params.target);

  if (!isValidHttpUrl(targetUrl)) {
    return res.status(400).send("Invalid logo URL");
  }

  const cached = channelPosterCache.get(targetUrl);
  if (cached && Date.now() - cached.createdAt < CHANNEL_POSTER_CACHE_TTL_MS) {
  ...
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), CHANNEL_POSTER_FETCH_TIMEOUT_MS);
  ...
  try {
    ...
    if (!buffer.length || buffer.length > 3 * 1024 * 1024) {
      throw new Error("Logo vazio ou demasiado grande");
    }

    let mime = String(response.headers.get("content-type") || "").split(";")[0].trim().toLowerCase();
    if (!/^image\/(?:png|jpeg|jpg|webp|gif|svg\+xml)$/.test(mime)) {
    ...
    const logoBox = getChannelPosterLogoBox(dimensions);
    const logoX = Math.round((600 - logoBox.width) / 2);
    const logoY = Math.round((900 - logoBox.height) / 2);
    const svg = `<?xml version="1.0" encoding="UTF-8"?>
<svg xmlns="http://www.w3.org/2000/svg" width="600" height="900" viewBox="0 0 600 900">
  <rect width="600" height="900" rx="28" fill="#111317"/>
  <image href="${dataUri}" x="${logoX}" y="${logoY}" width="${logoBox.width}" height="${logoBox.height}" preserveAspectRatio="xMidYMid meet"/>
</svg>`;

    channelPosterCache.set(targetUrl, { svg, createdAt: Date.now() });

    res.setHeader("Content-Type", "image/svg+xml; charset=utf-8");
    res.setHeader("Cache-Control", "public, max-age=86400, stale-while-revalidate=604800");
    return res.send(svg);
  } catch (error) {
    console.warn("PT•HUB Logos: falha ao criar poster do canal:", error.message);
  ...
  if (config.mode === "m3u") {
    ...
    if (config.m3uSource === "file") {
      ...
      if (config.m3uFileId) {
        ...
        return finalizeIPTVChannels(parseM3U(stored.content, config));

      }

      if (config.m3uFileData) {
        return finalizeIPTVChannels(parseM3U(config.m3uFileData, config));
      }
    ...
    return finalizeIPTVChannels(await fetchM3U(config.m3uUrl, config));
  }

  if (config.mode === "xtream") {
    const channels = await getXtreamChannels(config);
    return finalizeIPTVChannels(channels.map((channel) => ({
      ...channel,
          ...
          metas: channels.map(
            (channel) => {
              ...
              return {
                ...
                poster:
                  buildChannelPosterUrl(req, channel.logo || PT_HUB_LOGO),
      ...
      if (catalogId === "rtp-play") {
        ...
        return res.json({
          metas: channels.map((channel) => ({
            ...
            name: channel.name,
            posterShape: "poster",
            poster: buildChannelPosterUrl(req, channel.logo || PT_HUB_LOGO),
