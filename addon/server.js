...
const CONFIG_STORE_TTL_MS = 30 * 24 * 60 * 60 * 1000;
const CONFIG_STORE_MAX_BYTES = 512 * 1024;

const configStore = new Map();

function cleanupConfigStore() {
  const now = Date.now();

  for (const [token, entry] of configStore) {
    if (!entry || now - entry.createdAt > CONFIG_STORE_TTL_MS) {
      configStore.delete(token);
    }
  }
}

setInterval(cleanupConfigStore, 60 * 60 * 1000);

function createConfigToken() {
  return `c_${crypto.randomBytes(12).toString("hex")}`;
}

function getStoredConfig(token) {
  const entry = configStore.get(String(token || ""));

  if (!entry) {
    return null;
  }

  if (Date.now() - entry.createdAt > CONFIG_STORE_TTL_MS) {
    configStore.delete(String(token || ""));
    return null;
  }
  ...
  try {
    ...
    if (Buffer.byteLength(serialized, "utf8") > CONFIG_STORE_MAX_BYTES) {
      return res.status(413).json({
        success: false,
        error: "Configuração demasiado grande."
      });
    }

    const token = createConfigToken();
    ...
    console.error("Erro no Config Store:", error.message);
...
function encodeConfig(config) {
 ...
 function decodeConfig(value) {
  try {
    ...
    /*
     * Formato 3.0 curto:
     * c_<token> aponta para a configuração guardada no Config Store.
     */
    if (String(value).startsWith("c_")) {
      return getStoredConfig(value);
    }
